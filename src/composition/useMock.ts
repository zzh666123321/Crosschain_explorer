...
import crossTx from '@/mock/crossTx.json'
import crosstest from '@/mock/crosstest.json'
...
export const usecrossTxList = useFetchFactory<API.CrossListParams, typeof crossTx.data>(
    () => {
        const data = shuffle<typeof crossTx.data[0]>(crossTx.data);
        return Promise.resolve(wrapResponse(data));
    }
)
...
export const useCrossDetail = useFetchFactory<API.CrossDetailParams, typeof crosstest>(
    () => {
        const data = crosstest
        return Promise.resolve(wrapResponse({
            ...data,
            transactions: txs.data
        }))
